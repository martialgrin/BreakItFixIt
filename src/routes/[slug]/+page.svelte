<script>
  ...
  const portsA = [8000, 8001];
  const portsB = [8003, 8004];
  let ports = [];
  // firebase.initValues();
  ...
  function handler(event, state, type) {
    ...
    firebase.setValues(
      { x: x, y: y, state: state, ports: Object.assign({}, ports) },
  ...
  function changePorts(e) {
    ...
    firebase.setValues(
      { x: x, y: y, state: false, ports: Object.assign({}, ports) },
      "poster" + $page.url.pathname.slice(1)
    );
  }
...
<button id="button" on:click={changePorts}> Change ports </button>
